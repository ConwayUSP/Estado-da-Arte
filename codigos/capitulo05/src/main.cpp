...
int main() {
  ...
  unsigned char *dados =
      stbi_load("textures/container.jpg", &largura, &altura, &nCanais, 0);
  if (dados) {
  ...
  unsigned int projectionLoc =
      glGetUniformLocation(meuShaderInsano.ID, "projection");
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      model = glm::rotate(model, (glm::radians(angle) + timeAngle),
